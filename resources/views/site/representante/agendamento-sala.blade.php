...
<div class="d-block w-100">
    <p class="alert {{ $situacao['class'] }}" data-clarity-mask="True">{!! $situacao['message'] !!}</p>
</div>
@endif
            ...
            <div class="tab-pane container {{ $salas->isNotEmpty() ? 'active' : 'fade' }}" id="agendamento">
                <div class="list-group w-100 mt-2">
                    ...
                    <div class="list-group-item light d-block bg-info">
                        <p class="pb-0 branco" data-clarity-mask="True">Protocolo: <strong>{{ $item->protocolo }}</strong></p>
                        <p class="pb-0 branco" data-clarity-mask="True">Regional: <strong>{{ $item->sala->regional->regional }}</strong></p>
                        <p class="pb-0 branco" data-clarity-mask="True">
                            Sala: <strong>{{ $item->getTipoSala() }}</strong>
                            &nbsp;&nbsp;|&nbsp;&nbsp;Dia: <strong>{{ onlyDate($item->dia) }}</strong>
                            &nbsp;&nbsp;|&nbsp;&nbsp;Período: <strong>{{ $item->getPeriodo() }}</strong>
                        </p>
                        @if($item->isReuniao())
                        <p class="pb-0 branco" data-clarity-mask="True"><i class="fas fa-users text-dark"></i> Participantes: 
            ...
            <div class="tab-pane container {{ $salas->isEmpty() && $participando->isNotEmpty() ? 'active' : 'fade' }}" id="participando">
                <div class="list-group w-100 mt-2">
                    ...
                    <div class="list-group-item light d-block bg-secondary">
                        <p class="pb-0 branco" data-clarity-mask="True">Protocolo: <strong>{{ $participa->protocolo }}</strong></p>
                        <p class="pb-0 branco" data-clarity-mask="True">
                            Representante Responsável:  
                            &nbsp;&nbsp;CPF / CNPJ: <strong>{{ $participa->representante->cpf_cnpj }}</strong>
                            &nbsp;&nbsp;|&nbsp;&nbsp;Nome: <strong>{{ $participa->representante->nome }}</strong>
                        </p>
                        <p class="pb-0 branco" data-clarity-mask="True">Regional: <strong>{{ $participa->sala->regional->regional }}</strong></p>
                        <p class="pb-0 branco" data-clarity-mask="True">
