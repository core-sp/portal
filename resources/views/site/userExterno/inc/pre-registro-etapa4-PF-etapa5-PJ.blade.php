...
<div class="form-row mb-2">
    <div class="col-sm mb-2-576">
        ...
        <div class="form-check-inline">
            <label class="form-check-label">
                <input type="radio" 
                    class="{{ $classes[4] }} form-check-input {{ $errors->has('tipo_telefone') ? 'is-invalid' : '' }}" 
                    name="tipo_telefone" 
                    value="{{ $tipo }}" 
                    {{ (old('tipo_telefone') == $tipo) || (isset(explode(';', $resultado->tipo_telefone)[0]) && ($tipo == explode(';', $resultado->tipo_telefone)[0])) ? 'checked' : '' }} 
                />
                {{ $tipo }}
    ...
    <div class="col-sm mb-2-576">
        ...
        <input type="text"
            class="{{ $classes[4] }} form-control telefoneInput {{ $errors->has('telefone') ? 'is-invalid' : '' }}"
            name="telefone"
...
<div class="form-row mb-2">
    <div class="col-sm mb-2-576">
        ...
        <div class="form-check-inline">
            <label class="form-check-label">
                <input type="radio" 
                    class="{{ $classes[4] }} form-check-input {{ $errors->has('tipo_telefone_1') ? 'is-invalid' : '' }}" 
                    name="tipo_telefone_1" 
                    value="{{ $tipo }}" 
                    {{ (old('tipo_telefone_1') == $tipo) || (isset(explode(';', $resultado->tipo_telefone)[1]) && ($tipo == explode(';', $resultado->tipo_telefone)[1])) ? 'checked' : '' }} 
                />
                {{ $tipo }}
