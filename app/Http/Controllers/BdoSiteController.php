...
class BdoSiteController extends Controller
{
    ...
    public function anunciarVaga(AnunciarVagaRequest $request)
    {
        ...
        if($validated['idempresa'] == "0") {
            ...
            if($empresa->oportunidade_count > 0) {
                return redirect()
                    ->back()
                    ->with([
                        'message' => 'A empresa informada <strong>já possui uma vaga sob análise ou em andamento no Balcão de Oportunidades</strong> do Core-SP. Para solicitar nova inclusão, favor entrar em contato através do e-mail: <strong>aline.vieira@core-sp.org.br</strong> informando CNPJ, nome do responsável e telefone para contato.',
                        'class' => 'alert-danger'
                    ]);
            }
        }
        ...
        Mail::to(['aline.vieira@core-sp.org.br', 'desenvolvimento@core-sp.org.br'])->queue(new AnunciarVagaMail($oportunidade->idoportunidade));
