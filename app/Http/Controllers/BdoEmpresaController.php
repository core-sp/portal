...
class BdoEmpresaController extends Controller
{
    ...
    public function apiGetEmpresa($cnpj)
    {
        ...
        if($count > 0) {
            $message = 'A empresa informada <strong>já possui uma vaga sob análise ou em andamento no Balcão de Oportunidades</strong> do Core-SP. Para solicitar nova inclusão, favor entrar em contato através do e-mail: <strong>convenios.parcerias@core-sp.org.br</strong> informando CNPJ, nome do responsável e telefone para contato.';
            $class = 'alert-warning';
        } 
        else {
    ...
    public function tabelaCompleta($query)
    {
        ...
        $contents = $query->map(function($row){
            ...
            if(auth()->user()->can('delete', auth()->user())) {
                $acoes .= '<form method="POST" action="/admin/bdo/empresas/apagar/'.$row->idempresa.'" class="d-inline acaoTabelaAdmin">';
                $acoes .= '<input type="hidden" name="_token" value="'.csrf_token().'" />';
                $acoes .= '<input type="hidden" name="_method" value="delete" />';
                $acoes .= '<input type="hidden" class="cor-danger txtTabelaAdmin" value="Tem certeza que deseja excluir a empresa <i>' . $row->razaosocial . '</i>?" />';
                $acoes .= '<button type="button" class="btn btn-sm btn-danger" value="' . $row->idempresa . '">Apagar</button>';
                $acoes .= '</form>';
            }
